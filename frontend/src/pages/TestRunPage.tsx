...
export function TestRunPage() {
  ...
  const loadHistory = useCallback(
    () =>
      api.listTestRuns(testIdNum).then((list) => {
        setRuns(list)
  ...
  useEffect(() => {
    ...
    let finished = false
    const es = new EventSource(`/api/tests/runs/${selectedRunId}/stream`)
  ...
  if (error && !test) return <p>加载失败:{error}</p>
        ...
        <section className="flex min-h-0 flex-1 flex-col">
          <div className="mb-3 flex items-center justify-between">
            <div>
              ...
              {test && (
                <p className="text-sm" title={test.scriptPath}>
                  {test.scriptPath}
                </p>
              )}
            </div>
          ...
          {run && <Terminal run={run} scriptPath={test?.scriptPath} />}
        </section>
      </div>
